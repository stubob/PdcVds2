...
export default function RaceResultTable({
  ...
  const handleRiderChange = (sequence: number, selectedRider: Rider) => {
    setRows((prevRows) =>
      prevRows.map((row) =>
        row.sequence === sequence ? { ...row, selectedRider } : row
      )
    );
  };
    ...
    { field: "title", headerName: "Title", editable: false },
    {
      ...
      renderCell: (params) => (
        <RiderSelect
          ...
          defaultValue={
            filteredRiders.find((rider) => rider === params.row.rider) ||
            null
          }
  ...
  const HOUR_RECORD_TEMPLATE: TemplateType[] = [
    { title: "Special", sequence: 1, points: 100 },
  ];
  const NATIONAL_CHAMP_TEMPLATE: TemplateType[] = [
    { title: "National Champion", sequence: 1, points: 100 },
  ];
  const MINOR_RACE_TEMPLATE: TemplateType[] = [
  ...
  const MINOR_RACE_STAGE_TEMPLATE: TemplateType[] = [
    { title: "1", sequence: 1, points: 25 },
  ];
  const MINOR_RACE_FINAL_TEMPLATE: TemplateType[] = [
    { title: "1", sequence: 1, points: 100 },
    { title: "2", sequence: 2, points: 70 },
    { title: "3", sequence: 3, points: 50 },
    { title: "4", sequence: 4, points: 30 },
  ...
  const BEST_OF_THE_REST_TEMPLATE: TemplateType[] = [
  ...
  const BEST_OF_THE_REST_STAGE_TEMPLATE: TemplateType[] = [
    { title: "1", sequence: 1, points: 40 },
    { title: "2", sequence: 2, points: 20 },
    { title: "3", sequence: 3, points: 10 },
  ];
  const BEST_OF_THE_REST_FINAL_TEMPLATE: TemplateType[] = [
    { title: "1", sequence: 1, points: 150 },
    { title: "2", sequence: 2, points: 125 },
    { title: "3", sequence: 3, points: 100 },
    { title: "4", sequence: 4, points: 80 },
  ...
  const TOP_CLASSICS_TEMPLATE: TemplateType[] = [
  ...
  const TOP_STAGE_RACES_STAGE_TEMPLATE: TemplateType[] = [
  ...
  const TOP_STAGE_RACES_FINAL_TEMPLATE: TemplateType[] = [
    { title: "1", sequence: 1, points: 250 },
    { title: "2", sequence: 2, points: 200 },
    { title: "3", sequence: 3, points: 180 },
    { title: "4", sequence: 4, points: 160 },
  ...
  const GRAND_TOUR_STAGE_TEMPLATE: TemplateType[] = [
    { title: "1", sequence: 1, points: 100 },
  ...
  const GRAND_TOUR_FINAL_LEADER_JERSEY_TEMPLATE: TemplateType[] = [
  ...
  const MONUMENTS_AND_WORLDS_TEMPLATE: TemplateType[] = [
  ...
  useEffect(() => {
    const initData = async () => {
      ...
      let filteredData = mensRiderData;
      if (metadata?.type) {
        filteredData = womensRiderData;
      }
  ...
  const handleSave = async () => {
    for (const row of rows) {
      ...
      row.id = newRow.id; // Update the row with the new ID
    }
    ...
    <Card>
      <CardContent>
        ...
        <DataGrid
          ...
          columnVisibilityModel={{ id: false, sequence: false }}
        />
      </CardContent>
      <CardActions>
        <Button variant="contained" color="primary" onClick={handleSave} disabled={!!raceData}>
          Save
        </Button>
