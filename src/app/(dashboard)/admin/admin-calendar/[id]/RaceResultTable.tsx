...
import { updateRiderScores, updateScores, upsertRaceResult } from "../../../../prisma-queries";
...
interface RaceResultProps {
  ...
  export default function RaceResultTable({ isWomen, raceData, metadata }: RaceResultProps) {
    ...
    const columns: GridColDef[] = [
        ...
        { field: "riderName", headerName: "Rider", width: 500, editable: true, 
        renderCell: (params) => (
            <RiderSelect
                riderData={filteredRiders}
                defaultValue={filteredRiders.find(rider => rider.value === params.row.riderId) || null}
                onChange={(selectedRider) => {
                    const newRows = rows.map(row =>
                        row.sequence === params.row.sequence
                            ? { ...row, riderId: selectedRider.value }
                            : row
                    );
                    setRows(newRows);
                }
            ...
            />
                ...
                renderEditCell: (params) => <RiderSelect riderData={filteredRiders} defaultValue={params.value} 
                onChange={(selectedRider) => {
                    const newRows = rows.map(row =>
                        row.sequence === params.row.sequence
                            ? { ...row, riderId: selectedRider.value }
      ...
      const HOUR_RECORD_TEMPLATE: RaceResult[] = [
        { title: 'Special', sequence: 1, points: 100 },
    ]
    const NATIONAL_CHAMP_TEMPLATE: RaceResult[] = [
        { title: 'National Champion', sequence: 1, points: 100 },
    ]
    const MINOR_RACE_TEMPLATE: RaceResult[] = [
    ...
    const MINOR_RACE_STAGE_TEMPLATE: RaceResult[] = [
        { title: "1", sequence: 1, points: 25 },
    ];
    const MINOR_RACE_FINAL_TEMPLATE: RaceResult[] = [
        { title: "1", sequence: 1, points: 100 },
        { title: "2", sequence: 2, points: 70 },
        { title: "3", sequence: 3, points: 50 },
        { title: "4", sequence: 4, points: 30 },
    ...
    const BEST_OF_THE_REST_TEMPLATE: RaceResult[] = [
    ...
    const BEST_OF_THE_REST_STAGE_TEMPLATE: RaceResult[] = [
        { title: "1", sequence: 1, points: 40 },
        { title: "2", sequence: 2, points: 20 },
        { title: "3", sequence: 3, points: 10 },
    ];
    const BEST_OF_THE_REST_FINAL_TEMPLATE: RaceResult[] = [
        { title: "1", sequence: 1, points: 150 },
        { title: "2", sequence: 2, points: 125 },
        { title: "3", sequence: 3, points: 100 },
        { title: "4", sequence: 4, points: 80 },
    ...
    const TOP_CLASSICS_TEMPLATE: RaceResult[] = [
    ...
    const TOP_STAGE_RACES_STAGE_TEMPLATE: RaceResult[] = [
    ...
    const TOP_STAGE_RACES_FINAL_TEMPLATE: RaceResult[] = [
        { title: "1", sequence: 1, points: 250 },
        { title: "2", sequence: 2, points: 200 },
        { title: "3", sequence: 3, points: 180 },
        { title: "4", sequence: 4, points: 160 },
    ...
    const GRAND_TOUR_STAGE_TEMPLATE: RaceResult[] = [
        { title: "1", sequence: 1, points: 100 },
    ...
    const GRAND_TOUR_FINAL_LEADER_JERSEY_TEMPLATE: RaceResult[] = [
    ...
    const MONUMENTS_AND_WORLDS_TEMPLATE: RaceResult[] = [
  ...
  useEffect(() => {
    const initData = async () => {
        if(!raceData.length && metadata) {
            let template: RaceResult[] = [];
            ...
            const data = template.map((result, index) => ({
                ...result,
                raceId: metadata.id,
                sequence: index + 1,
            }));
        ...
        if(riderData){
            ...
            setFilteredRiders(filteredData.map(rider => ({ label: rider.name, value: rider.id })));
        }
    }
    initData();
}, [metadata, raceData.length, riderData, isWomen]);
...
function getRowId(row) {
  ...
  const handleSave = async () => {
    ...
    for (const row of rows) {
        await upsertRaceResult({ ...row, raceId: metadata.id });
    }
    const newData = await updateRiderScores();
    setRiderData(newData);
    await updateScores();
}  
  return (
    <Card>
    <CardContent>
        ...
        <DataGrid columns={columns} rows={rows} getRowId={getRowId} disableColumnFilter={true} disableColumnSorting={true}/>
    </CardContent>
    <CardActions>
